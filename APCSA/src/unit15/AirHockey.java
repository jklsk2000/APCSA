...
public class AirHockey extends Canvas implements KeyListener, Runnable
{
   ...
   public void paint(Graphics window)
   {
		...
		if (ball.didCollideLeft(leftg) || ball.didCollideRight(rightg))
		{
			ball.setXSpeed(0);
			ball.setYSpeed(0);
			
			if (ball.didCollideLeft(leftg)){
				rightscore = rightscore + 1;
			}
			if (ball.didCollideRight(rightg)){
		...
		if(ball.didCollideRight(leftg)||ball.didCollideLeft(rightg)){
